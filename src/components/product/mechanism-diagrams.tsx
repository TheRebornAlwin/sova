...
/**
 * "Where the knot sits" — an upper back seen from behind, the trapezius shaded,
 * and the reach-it-yourself spot marked at the base of the neck.
 */
export function TrapeziusDiagram() {
  return (
    <svg
      viewBox="0 0 240 196"
      className="w-full h-auto max-w-[300px] mx-auto"
      role="img"
      aria-label="The trapezius across the neck and shoulders, seen from behind, with the knot marked at the base of the neck"
    >
      {/* centered label, so it can never clip on either side */}
      <text x="120" y="16" textAnchor="middle" fontSize="12" fontWeight="600" fill={SAGE_DARK}>
        Where the knot sits
      </text>

      {/* upper back seen from behind: head, neck, shoulders sloping into arms,
          open at the bottom so it reads as a torso, not a dome */}
      <g fill="none" stroke={SAGE} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        <circle cx="120" cy="54" r="22" />
        <path d="M109 74 q11 10 22 0" />
        <path d="M112 80 C 86 86 58 104 48 138 L46 188" />
        <path d="M128 80 C 154 86 182 104 192 138 L194 188" />
      </g>

      {/* trapezius shading across the top of the shoulders */}
      <path
        d="M112 82 C 92 88 72 100 62 120 C 88 108 104 106 120 106 C 136 106 152 108 178 120 C 168 100 148 88 128 82 Z"
        fill={SAGE}
        opacity="0.18"
      />

      {/* the knot marker, at the base of the neck */}
      <circle cx="120" cy="100" r="10" fill="none" stroke={SAGE_DARK} strokeWidth="1.5" opacity="0.5" />
      <circle cx="120" cy="100" r="4.5" fill={SAGE_DARK} />
    </svg>
  );
}
